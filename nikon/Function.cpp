...
BOOL Search_Module( void* Path )
{
    ...
    CFStringRef		fileName = CFSTR("Type0006 Module.bundle");
        ...
        for(i = 0; i < pRefMod->ulChildCount; i ++)
        {
            ...
            for(j = 0; j < pRefSrc->ulChildCount; j ++)
            {
                ...
                for(k = 0; k < pRefItm->ulChildCount; k ++)
                {
                    ...
                    printf("in Close_Module pRefDat->pObject %p\n", pRefDat->pObject);
            ...
            if ( bRet == false )	return false;
            printf("in Close_Module pRefSrc->pObject %p\n", pRefSrc->pObject);
        ...
        case kNkMAIDCapability_LiveViewImageZoomRate:
            switch( ulValue ){
                case kNkMAIDLiveViewImageZoomRate_All:
                    ...
                    break;
                case kNkMAIDLiveViewImageZoomRate_13:
                    strcpy( psString, "13%" );
                    break;
...
char*	GetUnsignedString( ULONG ulCapID, ULONG ulValue, char *psString )
{
    ...
    case kNkMAIDCapability_MovRecInCardProhibit:
        ...
        if ( ulValue & kNkMAIDMovRecInCardProhibit_PCInBuf )
        {
            strcat( psString, "SDRAM data in buffer, " );
        }
        if ( ulValue & kNkMAIDMovRecInCardProhibit_CardInBuf )
        {
            strcat( psString, "Card data in buffer, " );
        }
...
BOOL SetWBPresetDataCapability( LPRefObj pRefSrc )
{
    ...
    while (1)
    {
        ...
        if ( 1 > stPresetData.ulPresetNumber || stPresetData.ulPresetNumber > 4 )
        {
...
BOOL GetLiveViewImageCapability( LPRefObj pRefSrc )
{
    ...
    if ( g_ulCameraType == kNkMAIDCameraType_D800 || g_ulCameraType == kNkMAIDCameraType_D800E )
    {
        ulHeaderSize = 384;
    }
...
BOOL IssueProcess( LPRefObj pRefSrc, ULONG ulCapID )
{
    ...
    // Start the process
    bRet = Command_CapStart( pSourceObject, ulCapID, (LPNKFUNC)CompletionProc, (NKREF)pRefCompletion, &pnResult );
...
BOOL SetProc( LPRefObj pRefObj )
{
    ...
    switch ( pRefObj->pObject->ulType  ) {
        ...
        case kNkMAIDObjectType_Source:
            ...
            printf("kNkMAIDObjectType_Source kNkMAIDCapability_EventProc registered\n");
            if( CheckCapabilityOperation( pRefObj, kNkMAIDCapability_EventProc, kNkMAIDCapOperation_Set ) ) {
